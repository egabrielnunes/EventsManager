...
@WebServlet(urlPatterns={"/AtualizarEventoPrivado", "/aepr" }, name="AtualizarEventoPrivado")

public class AtualizarEventoPrivado extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {//gera tela de atualizacao
		ServletOutputStream out = response.getOutputStream();
		out.println("<html><head><title>Criação de Evento Privado</title></head><body style='background: #fff8db;'>");
		out.println("<fieldset>");
		out.println("<form action='aepr' method='post'>");
		out.println("<fieldset>");
		out.println("Digite o id, do evento que deseja atualizar: <input type='text' name='id'><br><br>");
		out.println("Atualize seu Evento Particular: <br><br>");
		out.println("</fieldset>");
		...
		out.println("<input type='submit' value='Alterar Evento'><br>");
		out.println("</fieldset>");
	  ...
	  @SuppressWarnings("deprecation")
		protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {//recupera parametros e verifica
			ServletOutputStream out = response.getOutputStream();
			...
			if(diaA != "" && mesA != "" && anoA != "" && minutosA != "" && horaA != "" ){
				...
				Calendar calendar = Calendar.getInstance();//recupera data e hora de envio
...
//			
			...
			if(Facade.getInstancia().atualizarEventoPri(ep, id, cpfF) == true){
