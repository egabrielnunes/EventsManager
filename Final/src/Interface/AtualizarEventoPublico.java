...
@WebServlet(urlPatterns={"/AtualizarEventoPublico", "/aepu" }, name="AtualizarEventoPublico")

public class AtualizarEventoPublico extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public AtualizarEventoPublico() {
        super();
    }

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {//gera o formulario
		ServletOutputStream out = response.getOutputStream();

		out.println("<?php");
		out.println("if(isset($_REQUEST['enviarDados'])){");
		out.println("	$inputsCount = $_POST['inputsCount'];");//nao consegui de outra maneira.
		out.println("	for($x = 1; $x <= $inputsCount; $x++){");
		...
		out.println("function novoCampo(){");//gera inputs dinamicos
		...
		out.println("<form action='aepu' method='post' action='?enviarDados'>");
		out.println("<fieldset>");
		out.println("Digite o id, do evento que deseja atualizar: <input type='text' name='id'><br><br>");
		out.println("Atualize seu Evento Público: <br><br>");
		out.println("</fieldset>");
		...
		out.println("<br/>");
		out.println("<br/>");
		out.println("<input type='submit' value='Atualizar Evento'><br>");
		out.println("</fieldset>");
		out.println("</form></body></html>");
		
	}
	
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {//recupera, verifica e faz o cadastro
		ServletOutputStream out = response.getOutputStream();
		...
		if(Facade.getInstancia().atualizarEventoPu(epu, id, cpfF)){
			out.println("<html><head><meta http-equiv='refresh' content= '3; url=Layout'></head><body style='background: #fff8db;'>" +
					...
					"<b>Evento publico atualizado com sucesso.</b></div></body></html>");
		} else {
			out.println("<html><head><meta http-equiv='refresh' content= '3; url=Layout'></head><body style='background: #fff8db;'>" +
					"<div style='text-align: center; font-size:15px; top: 50px;position:absolute; right:40%; '> " +
					"<b>Falha na atualização.</b></div></body></html>");
		}
		...
		String aaa = request.getParameter("inputsCount");
		...
		for (int i = 0; i < a; i++) {
			try {
				...
				String idPublico = Facade.getInstancia().recuperaId(titulo);
				int id2 = Integer.parseInt(idPublico);
				...
				new EnviarMail(loginU, passU, valor, sub, sub1).start();
				...
				for (int j = 0; j < convidados.size(); j++) {
					for (Usuario u : convidados) {
						try {
							...
							Facade.getInstancia().cadastrarConvidado(cpf, email, id2);
						} catch (Exception e) {
