...
public class CriarUsuario extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {//gera formulario de insercao de usuario
		ServletOutputStream out = response.getOutputStream();
		out.println("<html><head><title>CriaÁ„o de Usu·rio</title></head><body style='background: #fff8db;'>");
		out.println("<b style='font-size:20px;'>Preencha os campos abaixo para a criaÁ„o de seu usu·rio:</b> <br><br>");
		out.println("<fieldset>");
		out.println("<form enctype='multipart/form-data' action='CriarUsuario' method='post'>");
		out.println("Login: <input type='text' name='login'> Deve iniciar com uma consoante e ter pelo menos 5 caracteres.<br><br>");
		out.println("Senha: <input type='password' name='senha'>  Deve ter pelo menos 6 caracteres.<br><br>");
	...
	private static String format(String pattern, Object value) { //formata o cpf com a mascara padrao
        MaskFormatter mask;
        try {
	...
	protected void doPost(HttpServletRequest request,
		HttpServletResponse response) throws ServletException, IOException { //recupera os parametros, valida e salva o usuario criado
		...
		if (isMultipart) {
			try {
				...
				out.println("<html><head><meta http-equiv='refresh' content= '3; url=CriarUsuario'></head><body style='background: #fff8db;'>" +
					...
					if(nome != "" || end != "" || end != "" || cpf != "" || login != "" || senha != "" 
						...
						if(senha.length() >= 6 && login.length() >= 5 && cpf.length() == 11) {
							if(bla.equals("a") || bla.equals("A") || bla.equals("e") || bla.equals("E") || bla.equals("i") 
								...
								if(Facade.getInstancia().cadastrarUsuario(us) == true) { // salva o usuario no banco
									out.println("<html><head><meta http-equiv='refresh' content= '3; url=0'></head><body style='background: #fff8db;'>" +
											"<div style='text-align: center; font-size:15px; top: 50px;position:absolute; right:40%; '> " +
											"<b>Cadastro Efetuado Com Sucesso!</b></div></body></html>");
								}else{
									out.println("<html><head><meta http-equiv='refresh' content= '3; url=CriarUsuario'></head><body style='background: #fff8db;'>" +
											"<div style='text-align: center; font-size:15px; top: 50px;position:absolute; right:40%; '> " +
